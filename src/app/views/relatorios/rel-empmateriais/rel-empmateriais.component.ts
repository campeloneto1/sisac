...
@Component({
  selector: 'app-rel-empmateriais',
  templateUrl: './rel-empmateriais.component.html',
  styleUrls: ['./rel-empmateriais.component.css']
})
export class RelEmpmateriaisComponent implements OnInit, OnDestroy {
  ...
  config4 = {
    displayFn:(item: any) => { 
        ...
        return item.serial+' ('+item.marca.nome+'/'+item.modelo.nome+')'; 
  ...
  formcad = new FormGroup({
    ...
    materia_tipo_id: new FormControl(''),   
  ...
  constructor(private session: SessionService,
    ...
    private materias: MateriaisService,
    private materiastipos: MateriaisTiposService,
  ...
  ngOnInit(): void {
    ...
    this.materias.index2().subscribe(data => {
      this.materiais$ = data;
    });

    this.materiastipos.index().subscribe(data => {
  ...
  getReport(){
    ...
    if(this.formcad.value.material){
      //@ts-ignore
    ...
    this.relatorios.getEmpMateriais(this.formcad.value).subscribe(data => {
      this.show = true;
