...
@Component({
  ...
  styleUrls: ['./patrimonios-tipos.component.css']
})
export class PatrimoniosTiposComponent implements OnInit, OnDestroy {
...
private router: Router,
    private tipospatrimonios: PatrimoniosTiposService) { 
   
        this.user = this.session.getUser();
        if(this.user.perfil.administrador){
          this.tipospatrimonios.index().subscribe(data => {
            this.data$ = data;
            this.dtTrigger.next();
          }); 
        }else{
  ...
  salvar(){
    if(this.formcad.value.id){
      //@ts-ignore
      this.tipospatrimonios.update(this.formcad.value, this.formcad.value.id).subscribe(data => {
        if(data == 1){
          this.refresh();
          this.formcad.reset();
      ...
      this.tipospatrimonios.store(this.formcad.value).subscribe(data => {
        if(data == 1){
          this.refresh();
          this.formcad.reset();
  ...
  confirm(){
    this.tipospatrimonios.destroy(this.excluir$.id).subscribe(data => {
      if(data == 1){
        this.refresh();    
