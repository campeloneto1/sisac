...
export class PaisesComponent implements OnInit,OnDestroy {
  ...
  salvar(){
    if(this.formcad.value.id){
      //@ts-ignore
      this.paises.update(this.formcad.value, this.formcad.value.id).subscribe(data => {
        if(data == 1){
          this.refresh();
          this.formcad.reset();
      ...
      this.paises.store(this.formcad.value).subscribe(data => {
        if(data == 1){
          this.refresh();
          this.formcad.reset();
  ...
  confirm(){
    this.paises.destroy(this.excluir$.id).subscribe(data => {
      if(data == 1){
        this.refresh();    
