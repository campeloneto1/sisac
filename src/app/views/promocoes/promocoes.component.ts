...
export class PromocoesComponent implements OnInit, OnDestroy {
  ...
  salvar(){
    ...
    if(this.formcad.value.id){
      //@ts-ignore
      this.usuariospromocoes.update(this.formcad.value, this.formcad.value.id).subscribe(data => {
        if(data == 1){
          this.refresh();
          this.formcad.reset();
      ...
      this.usuariospromocoes.store(this.formcad.value).subscribe(data => {
        if(data == 1){
          this.refresh();
          this.formcad.reset();
  ...
  confirm(){
    this.usuariospromocoes.destroy(this.excluir$.id).subscribe(data => {
      if(data == 1){
        this.refresh();    
